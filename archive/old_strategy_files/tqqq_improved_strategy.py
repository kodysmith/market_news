#!/usr/bin/env python3
"""
Improved TQQQ Options Protection Strategy Backtest
More realistic option pricing and position sizing

Strategy:
- Buy 50 shares of TQQQ every 2 weeks (reduced from 100)
- Buy put options to protect 50% of position size (reduced from 66%)
- Put options 90 days out, 2 standard deviations OTM (reduced from 4)
- Roll options when they reach ~30 days to expiration
- More realistic option pricing model
"""
...
class ImprovedTQQQStrategy:
    def __init__(self, 
                 initial_capital: float = 100000,
                 share_purchase_amount: int = 50,  # Reduced from 100
                 purchase_frequency_days: int = 14,
                 protection_ratio: float = 0.50,  # Reduced from 0.66
                 put_dte: int = 90,
                 roll_dte: int = 30,
                 otm_deviations: float = 2.0,  # Reduced from 4.0
                 start_date: str = '2020-01-01',
                 end_date: str = '2024-01-01'):
        """
        Initialize the improved TQQQ options protection strategy
        """
    ...
    def _calculate_put_strike(self, current_price: float, volatility: float, dte: int) -> float:
        """Calculate put strike price using more conservative approach"""
        if volatility <= 0 or dte <= 0:
            return current_price * 0.85  # Fallback to 15% OTM
        
        # More conservative strike calculation
        # Strike = S * (1 - volatility * sqrt(dte/365) * deviations * 0.5)
        strike_factor = 1 - (volatility * np.sqrt(dte/365) * self.otm_deviations * 0.5)
        strike = current_price * max(strike_factor, 0.7)  # At least 30% OTM
        
        # Ensure strike is reasonable
        min_strike = current_price * 0.5  # At least 50% of current price
        max_strike = current_price * 0.9  # At most 10% OTM
    ...
    def _estimate_put_premium(self, current_price: float, strike: float, volatility: float, dte: int) -> float:
        """More realistic put option premium estimation"""
        ...
        # More realistic Black-Scholes approximation
        ...
        put_price *= 1.2  # 20% spread
        put_price *= (1 + volatility * 0.1)  # Volatility premium
        
        # Cap the premium at 10% of stock price
        put_price = min(put_price, S * 0.1)
    ...
    def _calculate_portfolio_value(self, date: datetime, current_price: float) -> float:
        ...
        for position in self.options_positions:
            ...
            if current_dte > 0:
                ...
                time_value = position['premium'] * (current_dte / position['dte']) * 0.3
    ...
    def run_backtest(self) -> Dict:
        ...
        print("🚀 Starting Improved TQQQ Options Protection Strategy Backtest")
        print("=" * 60)
    ...
    def print_summary(self, results: Dict):
        ...
        print("\n" + "="*60)
        print("📊 IMPROVED TQQQ OPTIONS PROTECTION STRATEGY RESULTS")
        print("="*60)
...
def main():
    """Run the improved TQQQ options strategy backtest"""
    print("🏦 Improved TQQQ Options Protection Strategy Backtest")
    print("=" * 60)
    
    # Strategy parameters - more conservative
    strategy = ImprovedTQQQStrategy(
        initial_capital=100000,
        share_purchase_amount=50,  # Reduced from 100
        purchase_frequency_days=14,  # Every 2 weeks
        protection_ratio=0.50,  # Protect 50% of position (reduced from 66%)
        put_dte=90,  # 90 days to expiration
        roll_dte=30,  # Roll when 30 days left
        otm_deviations=2.0,  # 2 standard deviations OTM (reduced from 4)
    ...
    results_df.to_csv('tqqq_improved_strategy_results.csv', index=False)
    print(f"\n💾 Detailed results saved to 'tqqq_improved_strategy_results.csv'")
