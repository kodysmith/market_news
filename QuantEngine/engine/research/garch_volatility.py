...
class GARCHVolatilityForecaster:
    ...
    def forecast_volatility(
        ...
        try:
            ...
            # Confidence intervals using forecast standard error
            # For GARCH, forecast SE grows with horizon; use sqrt(2/T) as approximation
            T_est = max(len(returns), 100)
            se_ratio = np.sqrt(2.0 / T_est) * np.arange(1, horizon + 1) ** 0.5
            ci_lower = vol_forecast * np.exp(-1.96 * se_ratio)
            ci_upper = vol_forecast * np.exp(1.96 * se_ratio)
