...
class QuantBotIntegration:
    ...
    async def update_news_json(self):
        """
        Update the data/news.json file with live QuantBot-enhanced news feed
        """
        try:
            ...
            logger.info(f"✅ Updated data/news.json with {len(formatted_news)} items")
            return True

        except Exception as e:
            logger.error(f"Failed to update data/news.json: {e}")
            return False
...
async def update_news_feed():
    """API function to update data/news.json with live data"""
