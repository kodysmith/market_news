#!/usr/bin/env python3
"""
Update news.json with live data from QuantBot database
...
def update_news_json():
    ...
    try:
        ...
        print(f"✅ Updated news.json with {len(final_news)} items")
        ...
        print(f"❌ Failed to update news.json: {e}")
        import traceback
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Update news.json with QuantBot data')
    ...
    if args.stats:
        ...
        if success:
            print("\n🎉 news.json successfully updated with QuantBot data!")
            print("The market news app will now serve enhanced news with AI insights.")
        else:
            print("\n❌ Failed to update news.json")
            sys.exit(1)
