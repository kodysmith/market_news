#!/usr/bin/env python3
"""
Simplified TQQQ Options Protection Strategy Backtest
...
class SimpleTQQQStrategy:
    ...
    def _calculate_put_strike(self, current_price: float, volatility: float, dte: int) -> float:
        """Calculate put strike price using volatility-based approach"""
    ...
    def _estimate_put_premium(self, current_price: float, strike: float, volatility: float, dte: int) -> float:
        ...
        def norm_cdf(x):
            return 0.5 * (1 + np.tanh(0.8 * x))  # Approximation
        
        put_price = K * np.exp(-r * T) * norm_cdf(-d2) - S * norm_cdf(-d1)
        
        # Add realistic spread and minimum price
        put_price = max(put_price * 1.1, 0.01)
    ...
    def _buy_put_protection(self, date: datetime, current_price: float, volatility: float) -> Optional[Dict]:
        ...
        contracts_needed = int(np.ceil(shares_to_protect / 100))
