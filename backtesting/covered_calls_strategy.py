#!/usr/bin/env python3
"""
Covered Calls Strategy Backtest

Strategy:
- Start with $250,000 worth of shares
- Sell covered calls with positive Expected Value only
- Roll calls every 10 DTE when profitable
- Trade once per week
- Handle call exercise by adjusting share count
- Focus on income generation through options premium
"""
...
class CoveredCallsStrategy:
    def __init__(self,
                 ticker: str = 'TQQQ',
                 initial_capital: float = 250000,  # Start with $250k for shares
                 call_dte: int = 30,  # 30-day covered calls
                 min_ev_threshold: float = 0.02,  # Minimum 2% expected value
                 roll_dte_threshold: int = 10,  # Roll when 10 DTE left
                 trade_frequency_days: int = 7,  # Trade once per week
                 start_date: str = '2020-01-01',
                 end_date: str = '2024-01-01',
                 asset_data: Optional[pd.DataFrame] = None,
                 qqq_data: Optional[pd.DataFrame] = None):
        """
        Initialize the covered calls strategy for any asset

        Args:
            ticker: Stock ticker symbol
            initial_capital: Starting capital (used to buy initial shares)
            call_dte: Days to expiration for covered calls
            min_ev_threshold: Minimum expected value threshold (0.02 = 2%)
            roll_dte_threshold: Roll calls when this many DTE remain
            trade_frequency_days: Days between trading decisions
        ...
        self.call_positions = []  # Covered call positions
        self.trades = []
        self.portfolio_values = []
        self.dates = []

        # Covered calls specific parameters
        self.call_delta_target = 0.20  # Target 20% delta calls (reasonable moneyness)
    ...
    def _buy_initial_shares(self):
        """Buy initial shares with available capital"""
        if self.shares_owned > 0:
            return  # Already have shares

        # Get initial price
        initial_price = self.asset_data['Close'].iloc[0]
        shares_to_buy = int(self.cash // initial_price)
        ...
        if shares_to_buy > 0:
            ...
            print(f"🏗️  Bought {shares_to_buy} initial {self.ticker} shares at ${initial_price:.2f}")

    def _calculate_call_ev(self, current_price: float, strike: float, volatility: float, dte: int) -> float:
        """Calculate Expected Value for a covered call"""
        premium = self._estimate_call_premium(current_price, strike, volatility, dte)

        # Simple EV calculation: premium - expected exercise cost
        # For covered calls, EV is positive when premium > expected loss from exercise
        # We use a simple approximation: EV ≈ premium - (probability of exercise * (strike - current_price))
        ...
        # Probability of exercise (call finishes ITM)
        prob_exercise = norm_cdf(d2)

        # Expected value of exercise (if exercised, we sell at strike instead of keeping shares)
        expected_exercise_value = prob_exercise * (strike - current_price)

        # EV = premium received - expected cost of exercise
        ev = premium - expected_exercise_value

        return ev

    def _sell_covered_call(self, date: datetime, current_price: float, target_ev: float) -> Optional[Dict]:
        """Sell a covered call if EV is positive"""
        if self.shares_owned <= 0:
            return None

        # Get current volatility
        i = self.i_of[date]
        base_iv = np.clip(1.2 * self.iv_raw[i], 0.15, 0.35)

        # Find optimal strike with positive EV
        max_contracts = self.shares_owned // 100  # Can cover this many contracts

        best_strike = None
        best_premium = 0
        best_ev = 0

        # Try different strikes around target delta
        for delta in [0.15, 0.20, 0.25, 0.30]:
            strike = self._strike_for_call_delta(current_price, base_iv, self.call_dte, target_call_delta=delta)
            ev = self._calculate_call_ev(current_price, strike, base_iv, self.call_dte)

            if ev > target_ev and ev > best_ev:
                premium = self._estimate_call_premium(current_price, strike, base_iv, self.call_dte)
                if premium > best_premium:
                    best_strike = strike
                    best_premium = premium
                    best_ev = ev

        if best_strike is None or best_premium <= 0:
            return None

        # Sell the call
        contracts = min(max_contracts, 5)  # Limit to 5 contracts max for diversification
        premium_received = best_premium * 100 * contracts
        ...
        self.trades.append(trade)
        print(f"📞 Sold {contracts} covered calls at ${best_strike:.2f} (EV: {best_ev:.1%}) for ${best_premium:.2f}")

        return position

    def _roll_covered_calls(self, date: datetime, current_price: float) -> List[Dict]:
        """Roll or expire covered calls"""
        rolled_trades = []
        keep_calls = []

        for call_pos in self.call_positions:
            days_held = (date - call_pos['date_opened']).days
            current_dte = max(call_pos['dte'] - days_held, 0)

            # Check if exercised
            if current_price >= call_pos['strike']:
                # Call exercised - sell shares at strike
                shares_to_sell = call_pos['contracts'] * 100
                if self.shares_owned >= shares_to_sell:
                    proceeds = shares_to_sell * call_pos['strike']
                    self.cash += proceeds
                    ...
                    rolled_trades.append({
                        ...
                        'contracts': call_pos['contracts'],
                        'shares_sold': shares_to_sell,
                        'proceeds': proceeds
                    })
                    print(f"📞 Call exercised: sold {shares_to_sell} shares at ${call_pos['strike']:.2f}")
                continue

            # Roll if close to expiration and profitable
            elif current_dte <= self.roll_dte_threshold:
                # Buy back the call
                i = self.i_of[date]
                base_iv = np.clip(1.2 * self.iv_raw[i], 0.15, 0.35)
                buyback_price = self._estimate_call_premium(current_price, call_pos['strike'], base_iv, max(current_dte, 1))
                buyback_cost = buyback_price * 100 * call_pos['contracts']
                self.cash -= buyback_cost

                # Try to sell a new call
                new_call = self._sell_covered_call(date, current_price, self.min_ev_threshold)

                if new_call:
                    rolled_trades.append({
                        'date': date,
                        'action': 'ROLL_COVERED_CALL',
                        'old_strike': call_pos['strike'],
                        'new_strike': new_call['strike'],
                        'old_dte': current_dte,
                        'new_dte': self.call_dte,
                        'contracts': call_pos['contracts'],
                        'buyback_cost': buyback_cost,
                        'new_premium': new_call['premium'] * 100 * new_call['contracts']
                    })
                    print(f"🔄 Rolled {call_pos['contracts']} calls from ${call_pos['strike']:.2f} to ${new_call['strike']:.2f}")
                else:
                    # Just let it expire
                    rolled_trades.append({
                        'date': date,
                        'action': 'EXPIRE_COVERED_CALL',
                        'strike': call_pos['strike'],
                        'contracts': call_pos['contracts']
                    })
                    print(f"⏰ Let {call_pos['contracts']} calls expire at ${call_pos['strike']:.2f}")
            else:
                keep_calls.append(call_pos)
    ...
    def _calculate_portfolio_value(self, date: datetime, current_price: float) -> float:
        """Calculate total portfolio value for covered calls strategy"""
        shares_value = self.shares_owned * current_price
        options_value = 0.0

        # Get current volatility
        i = self.i_of[date]
        iv = np.clip(1.2 * self.iv_raw[i], 0.15, 0.35)

        # Handle call positions - check for exercise and mark to market
        calls_to_remove = []
        for j, p in enumerate(self.call_positions):
            ...
            if current_price >= p['strike']:
                ...
                if self.shares_owned >= shares_to_sell:
                    ...
                    print(f"📞 Call exercised: sold {shares_to_sell} shares at ${p['strike']:.2f}")
    ...
    def run_backtest(self) -> Dict:
        """Run the covered calls strategy backtest"""
        print(f"🚀 Starting {self.ticker} Covered Calls Strategy Backtest")
        print("=" * 60)

        # Buy initial shares
        self._buy_initial_shares()
        ...
        for i in range(N):
            ...
            if should_trade and self.shares_owned > 0:
                # Try to sell covered calls with positive EV
                call_trade = self._sell_covered_call(date, current_price, self.min_ev_threshold)
                if call_trade:
                    self.last_trade_date = date

            # Handle rolling/expiration of existing calls
            self._roll_covered_calls(date, current_price)
        ...
        results = {
            ...
            'call_positions': len(self.call_positions),
    ...
    def print_summary(self, results: Dict):
        ...
        print(f"Active Call Positions: {results['call_positions']}")
        ...
        if len(returns) > 0:
            ...
            print(f"Premium Collected: ${sum(t.get('proceeds', 0) for t in self.trades if t['action'] == 'SELL_COVERED_CALL'):,.0f}")
            print(f"Calls Exercised: {sum(1 for t in self.trades if t['action'] == 'CALL_EXERCISED')}")
