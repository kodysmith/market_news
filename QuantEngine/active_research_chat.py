#!/usr/bin/env python3
"""
Active Research Chat Interface for QuantEngine

This version actively fetches real data and performs research when it doesn't have
enough information to answer questions properly.
"""
...
class ActiveResearchChat:
    """
    Chat interface that actively researches and fetches data when needed
    """
    ...
    def __init__(self):
        ...
        self.data_cache = {}
        self.session = None
        print("🤖 Active Research QuantEngine Chat initialized")
    
    async def initialize(self):
        """Initialize the chat interface with data sources"""
        self.session = aiohttp.ClientSession()
        logger.info("✅ Active research capabilities initialized")
    ...
    async def fetch_asset_data(self, asset: str, days: int = 30) -> Dict[str, Any]:
    ...
    async def fetch_market_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for index in indices:
                data = await self.fetch_asset_data(index, 30)
                if data:
    ...
    async def fetch_news_sentiment(self, asset: str = None) -> Dict[str, Any]:
        """Fetch news sentiment for an asset or general market"""
    ...
    async def generate_data_driven_scenarios(self, parsed_question: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for asset in parsed_question['assets']:
            asset_data = await self.fetch_asset_data(asset, 30)
            if not asset_data:
                continue
            
            # Generate scenarios based on actual data
            scenarios.extend(await self.generate_asset_scenarios(asset, asset_data, market_data))
        
        # If no specific assets, generate general scenarios
        if not scenarios:
            scenarios = await self.generate_market_scenarios(market_data)
        
        return scenarios
    
    async def generate_asset_scenarios(self, asset: str, asset_data: Dict[str, Any], 
    ...
    async def generate_market_scenarios(self, market_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    async def generate_trading_strategy(self, asset: str, scenarios: List[Dict[str, Any]], 
    ...
    async def ask_question(self, question: str) -> Dict[str, Any]:
        ...
        scenarios = await self.generate_data_driven_scenarios(parsed_question)
        ...
        if any(word in question.lower() for word in ['strategy', 'trade', 'position', 'option']):
            for asset in parsed_question['assets']:
                asset_data = await self.fetch_asset_data(asset, 30)
                if asset_data:
                    strategies = await self.generate_trading_strategy(asset, scenarios, asset_data)
...
async def main():
    """Main entry point for active research chat"""
    print("🤖 Active Research QuantEngine Chat")
    print("=" * 50)
