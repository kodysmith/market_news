...
- ✅ `run_local.py` - Local runner script (all modes)
...
2. **Run a backtest** (no external dependencies needed):
   ```bash
   # Option 1: Using backtest runner
   python backtest/run_backtest.py --start-date 2024-01-01 --end-date 2024-01-31
   
   # Option 2: Using local runner
   python run_local.py --mode backtest --start-date 2024-01-01 --end-date 2024-01-31
   ```

3. **Set up paper trading locally** (optional, requires Supabase):
   ...
   - Run paper trading:
     ```bash
     # Option 1: Using local runner (recommended)
     python run_local.py --mode paper
     
     # Option 2: Using Flask app
     export TRADING_MODE=paper
     python main_live.py
     # Then POST to http://localhost:8080/run
     ```

4. **Test live mode locally** (⚠️ WARNING: uses real money):
   ```bash
